...
\noindent\textbf{A:}
%
CAT~\cite{currat} creates adversarial examples with gradually increasing
strength (\emph{i.e.}, PGD step) for adversarial training, while our
LGA gradually increases destination hardness $H_\mathsf{D}$ (with a
fixed PGD step) based on loss value.
%
SAT~\cite{smoothat} TODO.
%
Zhu~\emph{et al.}~\cite{optcontrolaml} TODO.
%
We will discuss these similar works in the revised manuscript.
