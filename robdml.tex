...
%
To battle against the attacks, some defenses incur gradient masking effect, but
often lead to a false sense of security~\cite{obfuscated}.
%
Defensive distillation~\cite{distill2} is compromised in~\cite{cw}.
%
It has been shown that ensemble of weak defenses is not
...
%
Of all defenses, adversarial training consistently remains to be one of the
most effective
methods~\cite{bilateral,advtrain-triplet,benchmarking,smoothat,trades,robustwrn,gowal,weightperturb},
but suffers from high training cost~\cite{freeat} and performance drop on
benign examples~\cite{odds,overfitting,geometry}.
