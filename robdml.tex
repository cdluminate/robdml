...
%
And the perturbation $\vr$ is updated as $\vr \leftarrow \vr + \alpha \Delta
...
To figure out the efficacy of HM with $H_\mathcal{D}$ calculated from benign
triplets, we adversarialy train models dataset using $\text{HM}[\mathcal{R},
...
cost~\footnote{Training cost is the number of forward-backward computation
cycle in each adversarial training iteration, and is calculated as $\eta+1$.},
as well as the R@1 performance on benign examples to reflect the
