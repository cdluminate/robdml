...
\begin{abstract}
	...
	% our starting point
	However, we argue that a model can learn from a strong adversary without
	model collapse, where the key for overall performance is triplet hardness.
	% HM framework
	To this end, Hardness Manipulation is proposed to adversarially perturb
	the training triplet to increase its hardness into a specified level,
	by referencing other benign triplets, or a pseudo-hardness function.
	% GA Adversary
	To further improve the model performance, we propose a Gradual Adversary to
	dynamicly change the hardness level to balance metric learning and
	adversarial learning.
	% Experiment
	The proposed method is validated on three commonly used deep metric
	learning datasets, namely CUB-200-2011, Cars-196, and Stanford Online
	Product.
	% Conclusion
	Comprehensive experimental results show our method outperforms the existing
	defense methods, while achieving clearly higher efficiency at a lower
	training cost.  
%
\end{abstract}
...
because the expectation of the destination hardness $E[H_\mathsf{D}]$ is expected to
decrease with a model being trained.

strong example when the embedding space is not in good shape.
...
\begin{equation}
	%
	g_\mathsf{B}(H(\cdots)) = H(\cdots) + \xi \cdot \bar{\ell}_{t-1}
	%
\end{equation}

\begin{equation}
	%
	g_\mathsf{SGA}() = -\gamma \cdot \bar{\ell}_{t-1}
	%
\end{equation}
