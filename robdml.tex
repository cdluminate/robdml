...
on three commonly used DML datasets: CUB-200-2011~\cite{cub200}, Cars-196~\cite{cars196},
...
semihard~\cite{facenet}, softhard[], distance~\cite{distance}, and the hardest negative sampling
(\ie, the negative sample with the largest loss value).
%
...
%
In order to figure out an empirically good combination of source and destination
hardness, we adversarially train models on the CUB-200-2011 dataset with
every possible combination, and compare their R@1 and ERS performance in
\cref{tab:desth}.
...
\subsection{Effectiveness of HM}
