...
%
Transferrability-based attacks relies on the observation that image-agnostic
...
%
Query-based attacks only need the logit score output or the label output from a
...
%
Defensive distillation~[] can be compromised by C\&W~\cite{cw}.
%
Ensemble of weak defenses is insufficient~[].
%
Defense can also be implemented as preprocessing~[] or reconstructing~[] the input
image, or a randomized process inside the network~[].
%
...
found effective~[][], and it consistently retains adversarial robustness
for the model.
