...
def projGradDescent(model: th.nn.Module, images: th.Tensor, labels: th.Tensor,
    ...
    if rinit:
        if images_orig.min() >= 0. and images_orig.max() <= 1.:
            if not B_UAP:
                images = images + eps * 2 * \
                    (0.5 - th.rand(images.shape, device=device))
            else:
                images = images + eps * 2 * \
            ...
            if not B_UAP:
                images = images + (eps / IMstd[:, None, None]).to(
                    device) * 2 * (0.5 - th.rand(images.shape, device=device))
            else:
    ...
    for iteration in range(pgditer):
        ...
        if B_UAP:
            ...
            with th.no_grad():
                aggrad = images.grad.mean(0, keepdim=True).detach().repeat(
        ...
        if pgditer > 1:
            if images_orig.min() >= 0. and images_orig.max() <= 1.:
                ...
                images.grad.data.copy_(
                    (alpha / IMstd[:, None, None]).to(device) * th.sign(images.grad))
            else:
                raise Exception
        else:
            if images.min() >= 0. and images.max() <= 1.:
                ...
                images.grad.data.copy_(
                    (eps / IMstd[:, None, None]).to(device) * th.sign(images.grad))
            else:
                raise Exception
        # update the input
        optimx.step()
        ...
        if not unbound:
            if images_orig.min() >= 0. and images_orig.max() <= 1.:
                ...
                images = th.min(images, images_orig +
                                (eps / IMstd[:, None, None]).to(device))
                images = th.max(images, images_orig -
                                (eps / IMstd[:, None, None]).to(device))
            else:
                raise Exception
        if images_orig.min() >= 0. and images_orig.max() <= 1.:
    ...
    if False:  # visualization
        for i in range(images.shape[0]):
            ...
            lab.subplot(121)
            lab.imshow(npxo, cmap='gray')
            lab.colorbar()
            lab.subplot(122)
            lab.imshow(npx, cmap='gray')
            lab.colorbar()
            lab.show()
    xr = images.detach()
